...
//` ColorButton messages
#define CBN_CLICK        WM_USER + 1050        // User clicked on button (first or second part) 


class CColorButton : public CButton
//class CColorButton : public CWnd
{
	DECLARE_DYNCREATE(CColorButton);

public:
	void DropDown(BOOL bKeyboardMode);

	CColorButton(void);
	virtual ~CColorButton(void);

	BOOL Create2(const RECT& rect, CWnd* pParentWnd, UINT uButtonID, UINT uDropDownID); //`
...
//	afx_msg LONG OnSelChange(UINT lParam, LONG wParam);
	...
	// Notes:		None.
	//***********************************************************************
	static void DrawDropDown(CDC* pDC, 
						  RECT* pRect, 
						  int iDirection = 0,
...
//	typedef CWnd _Inherited;
	BOOL m_bDrawEdge; //`
	afx_msg void OnMouseMove(UINT nFlags, CPoint ptClient);//`
