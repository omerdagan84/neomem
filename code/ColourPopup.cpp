...
CColourPopup::CColourPopup(CPoint p, COLORREF crColour, CWnd* pParentWnd,
						   ...
						   BOOL bKeyboardMode /* = FALSE */) //` added this
{
    Initialise();
	...
	m_bKeyboardMode = bKeyboardMode; //` added this (not actually used)
                          ...
                          LPCTSTR szCustomText  /* = NULL */)
{
	...
	//` The parent window deactivation thing bugged me too - so in CColourPopup::Create I 
	// dropped the WS_VISIBLE flag from CWnd::CreateEx(), and called ShowWindow() passing SW_SHOWNA.
//`    if (!CWnd::CreateEx(0, szClassName, _T(""), WS_VISIBLE|WS_POPUP, 
	...
	//` added this so keyboard will start on selected color
	m_nCurrentSel = m_nChosenColourSel; 

    // Capture all mouse events for the life of this window
	if (!m_bKeyboardMode) //` added this
		SetCapture();

	SetFocus(); //`added this so window will get keypresses (ie arrow keys!)
...
{    
	...
	if (m_bKeyboardMode) return; //` added this
...
// doesn't compile
// As written, the code will not support systems with multiple displays because of the use of the 
// SM_CXSCREEN and SM_CYSCREEN properties. When the control is on the non-primary display 
// of such a system, the popup window will appear on the primary display, which is decidedly 
// suboptimal behavior.
// Fortunately this is easily corrected by modifying the CColourPopup::SetWindowSize() function. 
// Change the last part of the function dealing with clipping the rectangle to the screen with the following code:
